    ...
    class Meta:
        ...
        widgets = {
            ...
            "name": forms.TextInput(
                attrs={"placeholder": "Nome", "class": "input-text"}
            ),
            "email": forms.EmailInput(
                attrs={"placeholder": "Email", "class": "input-text"}
            ),
            "phone_number": forms.TextInput(
                attrs={"placeholder": "Telefone", "class": "input-text"}
            ),
            "username": forms.TextInput(
                attrs={"placeholder": "Matrícula", "class": "input-text"}
            ),
            "birth_date": DateInput(
                attrs={"placeholder": "Data de nascimento", "class": "input-date"},
                format="%Y-%m-%d",
            ),
...
class UserEmployeeForm(UserCreationForm):
    password1 = forms.CharField(
        ...
        widget=forms.PasswordInput(
            attrs={"placeholder": "Senha", "class": "input-text"}
        ),
    )
    password2 = forms.CharField(
        label="Confirmar senha",
        widget=forms.PasswordInput(
            attrs={"placeholder": "Confirmar senha", "class": "input-text"}
        ),
    )
    gender = forms.ChoiceField(
        label="Sexo", choices=CustomUser.GENDER_CHOICES, widget=forms.RadioSelect()
    )
...
class UpdateScheduleForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {"hour": TimeInput(), "date": DateInput(format="%Y-%m-%d")}
...
class ScheduleForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "scheduled_by": forms.Select(
                attrs={
                    "hidden": True,
                }
            ),
            "matriculation": forms.Select(
                attrs={
                    "hidden": True,
                }
            ),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.request.GET.get("new_schedule"):
            ...
            self.fields["hour"] = forms.TimeField(
                widget=forms.Select(choices=TIME_CHOICES(*date))
            )

        else:
            try:
                if (
                    self.request.META["HTTP_REFERER"].split("?")[1].split("=")[0]
                    == "new_schedule"
                ):
                    self.fields["date"].initial = (
                        self.request.META["HTTP_REFERER"].split("?")[1].split("=")[1]
                    )
...
class ChangeScheduleForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.request.GET.get("selected_date"):
            ...
            self.fields["hour"] = forms.TimeField(
                widget=forms.Select(
                    choices=[(self.instance.hour, self.instance.hour)]
                    + TIME_CHOICES(data=self.instance.date)
                )
            )
...
class DoctorCreationForm(EmployeeCreationForm):
    ...
    class Meta(EmployeeCreationForm.Meta):
        ...
        widgets = {
            "speciality": forms.Select(attrs={"class": "select-primary"}),
            "birth_date": DateInput(
                attrs={"placeholder": "Data de nascimento", "class": "input-date"}
            ),
        }
...
class EmployeeChangeForm(UserChangeForm):
    ...
    username = forms.CharField(
        help_text="", widget=forms.TextInput(attrs={"readonly": True})
    )
    gender = forms.ChoiceField(
        label="Sexo",
        disabled=True,
