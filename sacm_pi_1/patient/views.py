...
from sacm_pi_1.clinic.models import (
    Clinic,
    CustomUser,
    Schedule,
    ScheduleStatus,
...
class AboutPageView(TemplateView):
    ...
    extra_context = {"about": True}

    def get(self, request, *args, **kwargs):
        kwargs.update(
            {
                "notification": Schedule.objects.filter(
        ...
        kwargs.update(
            {
                "notification_count": len(kwargs["notification"])
                - len(kwargs["notification"].filter(status=concluido))
            }
        )
...
def agenda(request):
    ...
    list_schedule = Schedule.objects.filter(matriculation=request.user.pk).order_by(
        "id"
    )
    ...
    if request.GET.get("date"):
        list_schedule = Schedule.objects.filter(
            date=request.GET.get("date"), matriculation=request.user.pk
        )
...
def novo_agendamento(request):
    ...
    form.fields["hour"] = forms.TimeField(
        widget=forms.Select(choices=TIME_CHOICES(data=dt(2023, 1, 16).date()))
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            model.objects.create(
                ...
                status=ScheduleStatus.objects.get(name="Pendente"),
                id_system=Clinic.objects.first(),
            )
...
def atualizar_agendamento(request, pk):
    ...
    if request.method == "POST":
        ...
        initial_data = {"date": instance.date, "speciality": instance.speciality}

        form = CreateScheduleForm(instance=instance, initial=initial_data)
        form.fields["hour"] = forms.TimeField(
            widget=forms.Select(
                choices=[((instance.hour), (instance.hour))]
                + TIME_CHOICES(data=instance.date)
            )
        )
        form.fields["speciality"] = forms.ModelChoiceField(
